...
- `--with-openapi` — Annotate all handlers and DTOs with `utoipa`, generate `ApiDoc` struct, wire Scalar UI
- `--with-wasm-client` — Generate a `<prefix>-client` crate via `progenitor` from the OpenAPI spec
...
7. `openapi` — `#[derive(ToSchema)]` on DTOs, `#[utoipa::path]` on handlers, `ApiDoc` struct, Scalar endpoint *(if `--with-openapi`)*
8. `wasm-client` — `<prefix>-client` crate generated by `progenitor` from the emitted spec *(if `--with-wasm-client`)*
...
# With --with-openapi:
Files also created (3):
  auth-api/src/openapi.rs             — ApiDoc struct, with_scalar() helper
  auth-api/src/dto/requests.rs        — CreateUserRequest, UpdateUserRequest (with ToSchema + examples)
  auth-api/src/dto/responses.rs       — UserResponse, UsersResponse, ErrorResponse (with ToSchema)

Cargo.toml updated (1 additional):
  auth-api/Cargo.toml — added: utoipa (axum_extras,uuid,chrono), utoipa-axum, utoipa-scalar

Scalar UI: http://localhost:3000/scalar
Spec JSON:  http://localhost:3000/openapi.json

# With --with-wasm-client (requires --with-openapi):
Files also created (2):
  auth-client/Cargo.toml              — progenitor-client, reqwest (WASM + native features)
  auth-client/build.rs                — regenerates src/generated.rs from docs/openapi.json
